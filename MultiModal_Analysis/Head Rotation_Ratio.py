...
while cap.isOpened():
    ...
    if results.multi_face_landmarks:
        ...
        data.append([frame_number, delta_ratio])
        frame_number += 1
...
# 데이터 프레임 만들기.
ratios_df = pd.DataFrame(data, columns=['Frame', 'Ratio_delta'])

# 엑셀 파일로 저장하는 구간.
excel_file_path = 'C:/Users/HarryAnnie/Downloads/A1_result/delta_ratio_A1.xlsx'
ratios_df.to_excel(excel_file_path, index=False)

plt.figure(figsize=(10, 5))
plt.plot(ratios_df['Frame'], ratios_df['Ratio_delta'], label='Delta Ratio')
plt.xlabel('Frame')
plt.ylabel('Delta Ratio')
plt.title('Delta Ratio over Frames')
plt.legend()
plt.grid(True)
plt.savefig('C:/Users/HarryAnnie/Downloads/A1_result/delta_ratios_A1_plot.png')
plt.show()